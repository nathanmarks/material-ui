...
describe('<Chip />', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders a delete icon after the label when onRequestClose is provided', () => {
      const wrapper = themedShallow(
        <Chip onRequestClose={sinon.spy()}>Label</Chip>
      );
  ...
  describe('callbacks', () => {
    it('triggers onRequestClose when the delete icon is clicked', () => {
      const handleRequestClose = sinon.spy();
      const wrapper = themedShallow(
        <Chip onRequestClose={handleRequestClose}>Label</Chip>
      );
      wrapper.childAt(1).simulate('click', {stopPropagation() {}});
      assert.ok(handleRequestClose.calledOnce);
      wrapper.childAt(1).simulate('touchTap', {stopPropagation() {}});
      assert.ok(handleRequestClose.calledTwice);
    });
